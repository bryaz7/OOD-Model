...
def train_process(args):
    ...
    train_df, num_classes = load_dataset('prompts_train', data_type=data_type)
    test_df, _ = load_dataset('prompts_test', data_type=data_type)
    eval_df, _ = load_dataset('prompts_val', data_type=data_type)
...
def finetune_bcad(args):
    ...
    if data_type == "sst":
        ...
        train_df = load_dataset('prompts_train', data_type=data_type)
        test_df = load_dataset('prompts_test', data_type=data_type)
        eval_df = load_dataset('prompts_val', data_type=data_type)
        for df in [train_df, test_df, eval_df]:
            set_binary_label(df, indomain=True) # set label to one (in domain)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_type', default='full', choices = ['courses', 'sst'])
